...
1. **Title** — First decide from the transcript whether this was a regular club MEETING (monthly gathering with workshop/technique/business) or an EVENT (a specific activity recorded for remote members — e.g. a Mead Day, a brew day, a workshop session, a brewery visit). Title accordingly: a meeting is "WCB Monthly Meeting — <date>"; an event uses the event's real name, e.g. "WCB Mead Day — <date>" or "WCB Brew Day: Pilsner — <date>". Include location if the transcript states one. Use an <h1> for the title. If it's genuinely ambiguous, default to "WCB Meeting — <date>".
   The title MUST be a clean, publishable title and NOTHING ELSE. Hard rules:
   - NEVER write meta-commentary or uncertainty into the title. Do NOT append parentheticals like "(date not stated in transcript)", "(date unknown)", "(approximate)", or any note about what you could or couldn't find. If the date is not clearly present in the transcript, simply OMIT the date — "WCB Monthly Meeting" with no date is correct and fine (a human supplies the date separately).
   - Do NOT include meeting-platform/recording artifacts in the title: no "Local Time", "Recording", "Transcript", timezones, "Zoom"/"Meet"/"Google Meet", timestamps, or UI labels. Note: a word like "Local Time" here is a recording-header artifact — never a venue; do not treat it as a location.
   - Output exactly ONE <h1> line for the title. Do not emit a second title line, a subtitle, or a duplicate heading.
